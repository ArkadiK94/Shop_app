...
deleteBtns.forEach(btn=>{
  btn.addEventListener("click",(e)=>{
    ...
    const csrf = target.parentElement.querySelector("input[name='_csrf']").value;
    const productId = target.parentElement.querySelector("input[name='productId']").value;
    ...
    .then(result =>{
      if(deleteRoute === "/cart/"){
        ...
        if(target.closest("div.grid").querySelectorAll("article").length > 1){
